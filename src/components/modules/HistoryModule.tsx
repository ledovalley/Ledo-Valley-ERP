...
export default function HistoryModule({ historyList = [], systemUser, onUndoFinalization }: HistoryModuleProps) {
  ...
  const kpiData = useMemo(() => {
    ...
    filteredHistory.forEach(h => {
      if (h.type === 'Finalized Process' && h.details) {
        totalYields += h.details.totalOutputQuantity || 0;
    ...
    return { totalYields, totalVariance, minDate, maxDate };
  }, [filteredHistory]);
  ...
  return (
    <>
      <div className="py-4 bg-[#F0F5F9]/30 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white border border-[#0B172B]/8 rounded-xl p-4 flex items-center justify-between shadow-sm">
          <div>
            <h4 className="text-[10px] font-black uppercase tracking-widest text-[#0B172B]/50 mb-1">Total Validated Yields</h4>
            <div className="text-2xl font-black text-[#0B172B]">{kpiData.totalYields.toFixed(2)} <span className="text-sm font-bold text-[#0B172B]/50">kg</span></div>
          </div>
          <div className="text-right flex flex-col justify-end h-full">
            <span className="text-xs font-bold text-[#0B172B]/60 bg-[#F0F5F9] px-2 py-1 rounded">
              {kpiData.minDate ? `${kpiData.minDate} to ${kpiData.maxDate}` : 'No data'}
            </span>
          </div>
        </div>
        <div className="bg-white border border-[#0B172B]/8 rounded-xl p-4 flex items-center justify-between shadow-sm">
          <div>
            <h4 className="text-[10px] font-black uppercase tracking-widest text-[#0B172B]/50 mb-1">Total Variance / System Loss</h4>
            <div className={`text-2xl font-black ${kpiData.totalVariance > 0 ? 'text-rose-600' : 'text-[#009965]'}`}>{kpiData.totalVariance.toFixed(2)} <span className={`text-sm font-bold ${kpiData.totalVariance > 0 ? 'text-rose-600/50' : 'text-[#009965]/50'}`}>kg</span></div>
          </div>
          <div className="text-right flex flex-col justify-end h-full">
            <span className="text-xs font-bold text-[#0B172B]/60 bg-[#F0F5F9] px-2 py-1 rounded">
              {kpiData.minDate ? `${kpiData.minDate} to ${kpiData.maxDate}` : 'No data'}
      ...
      <div className="bg-white rounded-2xl border border-[#0B172B]/8 h-full flex flex-col overflow-hidden relative shadow-[0_10px_30px_rgba(11,23,43,0.04)]">
        ...
        <div className="flex-1 overflow-auto bg-[#F0F5F9]/20">
